...
#define STOP_BUCKETSNUM 1009
#define DATA_BUCKETSNUM 10007

static void Welcome(const char *welcomeTextFileName);
static void BuildIndices(const char *feedsFileName, hashset *stopWords, hashset *visited, hashset *words);
static void ProcessFeed(const char *remoteDocumentName, hashset *stopWords, hashset *visited, hashset *words);
static void PullAllNewsItems(FILE *datastream, hashset *stopWords, hashset *visited, hashset *words);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stopWords, hashset *visited, hashset *words);
...
int VectorCmp(const void* vec1, const void* vec2);
...
int WordsCmp (const void* string1, const void* string2);
int WordsHash(const void* s, int numBuckets);
void VecFree(void* vec);
void WordsFree(void* elem);
static void allStopWords(const char *const kStopWords, hashset *stopWords);
void wordOccurence(const char* word, const char* title, const char* url,hashset* stopWords,hashset* words);
static ArticleStruct* FindArticle(vector *occ, const char *articleURL);
static int ArticleCmp(const void* first, const void* second);
...
int main(int argc, char **argv) {
  ...
  hashset stopWords;
  HashSetNew(&stopWords,sizeof(char*),STOP_BUCKETSNUM,StringHash,StringCmp,StringFree);
  allStopWords(kStopWords,&stopWords);

  hashset visited;
  HashSetNew(&visited,sizeof(char*),DATA_BUCKETSNUM,StringHash,StringCmp,StringFree);

  hashset words;
  HashSetNew(&words,sizeof(WordStruct),DATA_BUCKETSNUM,WordsHash,WordsCmp,WordsFree);


  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1],&stopWords,&visited,&words);
...
static void allStopWords(const char *const kStopWords, hashset *stopWords){
  FILE *infile;
  char buff[1024];
  streamtokenizer stkn;

  infile = fopen(kStopWords,"r");
  ...
  STNew(&stkn,infile,kTextDelimiters,true);
  while(STNextToken(&stkn,buff,sizeof(buff))){
    char* str = strdup(buff);
    if (HashSetLookup(stopWords, &str) == NULL) {
...
int WordsCmp (const void* string1, const void* string2){
  WordStruct* word1 = (WordStruct*) string1;
  WordStruct* word2= (WordStruct*)string2;

  return strcasecmp(word1->key, word2->key);
}


int WordsHash(const void* s, int numBuckets){
  WordStruct* word = (WordStruct*) s;
  return StringHash(&word->key,numBuckets);
}

void WordsFree(void* elem){
  WordStruct* words = (WordStruct*)elem;

  free(words->key);
  VectorDispose(&words->occ);
}


int VectorCmp(const void* vec1, const void* vec2){

  if(((ArticleStruct*)vec1)->frequency > ((ArticleStruct*)vec2)->frequency ){
    return -1;
  }else if(((ArticleStruct*)vec1)->frequency < ((ArticleStruct*)vec2)->frequency){
...
void VecFree(void* vec){
  ArticleStruct* article = (ArticleStruct*)vec;
  free(article->title);
  free(article->URL);
}

static int ArticleCmp(const void* first, const void* second){
   if(((ArticleStruct*)first)->frequency > ((ArticleStruct*)second)->frequency ){
    return -1;
  }else if(((ArticleStruct*)first)->frequency < ((ArticleStruct*)second)->frequency){
  return 1;
}
return 0;
}
...
static void BuildIndices(const char *feedsFileName,hashset* stopWords,hashset* visited,hashset* words) {
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName,stopWords,visited,words);
  }
...
static void ProcessFeed(const char *remoteDocumentName,hashset* stopWords,hashset* visited,hashset* words) {
  ...
  PullAllNewsItems(tmpFeed,stopWords,visited,words);
  fclose(tmpFeed);
}
...
static void PullAllNewsItems(FILE *dataStream,hashset *stopWords, hashset *visited, hashset *words) {
  ...
  while (GetNextItemTag(
    ...
    ProcessSingleNewsItem(&st,stopWords,visited,words);
  }
...
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stopWords, hashset *visited, hashset *words){
  ...
  if (strncmp(articleURL, "", sizeof(articleURL)) == 0){
    ...
    // char* urlPtr = articleURL;
    // char* urlPtr2 = articleURL;
    // if(HashSetLookup(visited,&urlPtr) == NULL){
    //   urlPtr2 = strdup(articleURL);
    //   HashSetEnter(visited,&urlPtr2);
...
static ArticleStruct* FindArticle(vector *occ, const char *articleURL) {
    for (int i = 0; i < VectorLength(occ); i++) {
        ArticleStruct *art = (ArticleStruct *)VectorNth(occ, i);
        if (strcasecmp(articleURL, art->URL) == 0) {
            return art;
        }
    }
    return NULL;
...
void wordOccurence(const char* word, const char* title, const char* url,hashset* stopWords,hashset* words){
  ...
  WordStruct findingWs;

  findingWs.key = (char*)word;
  WordStruct* foundWs = HashSetLookup(words, &findingWs);

  if(foundWs == NULL){
    WordStruct newWs;
    newWs.key = strdup(word);
    VectorNew(&newWs.occ, sizeof(ArticleStruct), VecFree, 4);

    ArticleStruct newArt;
    newArt.URL = strdup(url);
    newArt.frequency = 1;
    newArt.title = strdup(title);
    VectorAppend(&newWs.occ,&newArt);
    HashSetEnter(words,&newWs);
  }else{
   ArticleStruct *str = FindArticle(&foundWs->occ,url); 

   if(str!=NULL){
    str->frequency+=1;
    return;
    }else{
    ArticleStruct newArticle;
    newArticle.frequency = 1;
    newArticle.title = strdup(title);
    newArticle.URL= strdup(url);
    VectorAppend(&foundWs->occ,&newArticle);
    }
  }  
}
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWellFormed(word)) {
        wordOccurence(word,articleTitle,articleURL,stopWords,words);
        numWords++;
...
static void ProcessResponse(const char *word, hashset *stopWords, hashset *words) {
    if(WordIsWellFormed(word))
    {
       ...
       WordStruct srch;
       srch.key = (char*)word;
       WordStruct *found = (WordStruct*) HashSetLookup(words,&srch);

       if(found!=NULL){
        VectorSort(&found->occ,ArticleCmp);

        int limit = 0;
        if(VectorLength(&found->occ) > 10){
          limit = 10;
        }else{
          limit = VectorLength(&found->occ);
        }

        for(int i = 0 ; i < limit; i++){
          ArticleStruct* article = (ArticleStruct*)VectorNth(&found->occ,i);
          if(article->frequency == 1){
            printf("%2d.) \"%s\" [search term occurs %d time]\n", i + 1, article->title, article->frequency);
          }else{
