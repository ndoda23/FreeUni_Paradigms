...
void bfs(string& source, string& target, imdb& db){
  ...
  while(!partialPaths.empty() && partialPaths.front().getLength()<=5){
    ...
    vector<film> films;
    db.getCredits(currPath.getLastPlayer(),films);
    ...
    for(size_t i = 0; i < films.size(); i++){
      ...
      for(size_t k = 0; k < actors.size(); k++){
        if(actors[k] == target){
          path secPath = currPath;
          secPath.addConnection(films[i], actors[k]);
          cout << endl << secPath << endl;
          return;
        }
        
        if(previouslySeenActors.find(actors[k]) != previouslySeenActors.end()) {
          continue;  
        }
        
        previouslySeenActors.insert(actors[k]);
        path secPath = currPath;
        secPath.addConnection(films[i], actors[k]);
        partialPaths.push(secPath);  
      }
    }
  }
