...
(define (distance-product pt pts)
  (let ((filtered-pts (remove pt pts)))
    (cond ((null? filtered-pts) 1)
          (else (apply * (map (lambda (other-pt) (dist pt other-pt)) 
                              filtered-pts))))))


(define (rate-points pts)
  (map (lambda (point) 
         (list (distance-product point pts) point)) 
       pts))


(define (partition pivot num-list compare-fn)
  (cond ((null? num-list) '(() ()))
        (else (let ((split-of-rest (partition pivot (cdr num-list) compare-fn)))
                (cond ((compare-fn (car num-list) pivot)
                       (list (cons (car num-list) (car split-of-rest)) 
                             (cadr split-of-rest)))
                      (else (list (car split-of-rest) 
                                  (cons (car num-list) (cadr split-of-rest)))))))))

(define (quicksort compare-fn num-list)
  (cond ((<= (length num-list) 1) num-list)
        (else (let ((split (partition (car num-list) (cdr num-list) compare-fn)))
                (append (quicksort compare-fn (car split)) 
                        (list (car num-list)) 
                        (quicksort compare-fn (cadr split)))))))


(define (sort-points rated-pts)
  (quicksort (lambda (pt1 pt2) (< (car pt1) (car pt2))) 
             rated-pts))


(define (clumped-points pts)
  (let* ((sorted-rated (sort-points (rate-points pts)))
         (half-count (quotient (length sorted-rated) 2))
         (top-half (prefix-of-list sorted-rated half-count)))
    (map (lambda (rated-pt) (cadr rated-pt)) top-half)))
...
(define (average-point pts)
  (let* ((count (length pts))
         (x-coords (map x pts))
         (y-coords (map y pts))
         (center-x (/ (apply + x-coords) count))
         (center-y (/ (apply + y-coords) count))
         (avg-center (make-pt center-x center-y)))
    (list (distance-product avg-center pts) avg-center)))


(define (best-estimate guess)
  (let* ((all-intersections (intersection-points guess))
         (clustered (clumped-points all-intersections)))
    (average-point clustered)))


(define (where-am-i distances stars)
  (let* ((all-guesses (all-guesses distances stars))
         (all-estimates (map best-estimate all-guesses)))
    (sort-points all-estimates)))
